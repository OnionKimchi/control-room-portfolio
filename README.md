...
| --- | --- |
| ![Discord control room](assets/screenshots/discord-conversation-preview.png) | ![Frontend console](assets/screenshots/frontend-console.png) |
| 사용자 명령과 멀티턴 대화가 일어나는 화면 | multi-agent prompt/persona 상태를 관리하는 운영 콘솔 |

| Runner App | Workflow Debugger |
| --- | --- |
| ![Runner app](assets/screenshots/runner-app.png) | ![Workflow debugger](assets/screenshots/workflow-run-trace.png) |
| 로컬 Docker stack, console, runner 상태를 확인하는 Windows 앱 | backend node graph, run, trace 구조를 보여주는 workflow debugger |

| Model Profiles | Secret References |
| --- | --- |
| ![Model profiles](assets/screenshots/model-profiles.png) | ![Secret references](assets/screenshots/secret-references.png) |
| provider, model, parameter를 설정으로 교체하는 model profile 관리 화면 | AI와 frontend는 placeholder만 다루고 실제 key는 backend가 숨기는 secret reference 화면 |
